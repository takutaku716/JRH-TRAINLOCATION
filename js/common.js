...
$(function ($) {
	...
	if ($("header").length) {
		...
		// ヘッダーHTMLのURLを算出する。
		const url = (lang === "ja" ? "./call_common_header.html" : "http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/trainlocation/call_common_header_" + lang + ".html") + "?" + now;
		// ヘッダーを埋め込み表示する。
		$("header").load(url, () => {
			// ヘッダーに「現在日時」を表示する。
			...
			$(document).on("click", "#headerHowToUse, #commonAttention #attentionGuide", () => {
				...
				if (!$("#guideDetail").children().length) {
					// ｢使い方｣HTMLのURLを算出する。
					const url = (lang === "ja" ? "./guide/call_guide.html" : "http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/trainlocation/call_guide_" + lang + ".html")
					// ｢使い方｣を埋め込む。
