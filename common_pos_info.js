...
function getNearestStation(lat, lon, maxDistance, _isTop) {
	...
	$.when(
		$.getJSON("http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/webunkou/json/master/eki_master.json?" + now),
		$.getJSON("http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/webunkou/json/master/eki_jogai_master.json?" + now),
		$.getJSON("http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/webunkou/json/master/eki_coordinates_master.json?" + now)
	)
...
function load_rosen_html(_key, _isTop) {
	// キャッシュバスター値を生成する。(UNIX元期からの経過ミリ秒数を右に16ビットシフトした値。2の16乗＝65536ミリ秒≒約1分間隔でキャッシュを無効化する)
	const now = Date.now() >>> 16;
	$.getJSON("http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/webunkou/json/master/eki_rosen_master.json?" + now, function (rosenData) {
