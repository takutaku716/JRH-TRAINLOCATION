...
window.onload = function(){
	...
	let popup_url = lang == "ja" ? "http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/trainlocation/CMUNKOU/inc_location_popup.html?" + now : "http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/trainlocation/CMUNKOU/inc_location_popup_" + lang + ".html?" + now;
	$.ajax({
		url: popup_url,
		detaType: "html",
...
$(function ($) {
	...
	$(document).on("click", ".express-train-contents", function() {
		...
		// 最新の列車運行情報を取得する。
		$.when(
			$.getJSON("http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/webunkou/json/daiya/daiya_00" + (lang === "ja" ? "" : "_" + lang) + ".json?" + mstNow),
			$.getJSON("http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/trainlocation/json/express/now/express_now.json?" + trnNow)
		)
...
function set_station_list(_param_rosen, _scrollKey, _callback) {
	...
	let now = Date.now() >>> 10;
	let rosen_html = lang == "ja" ? `./rosen/rosen_${_param_rosen}.html` : `http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/trainlocation/rosen_${_param_rosen}_${lang}.html`;
	let maintenance_html = lang == "ja" ? "./mainte/rosen_maintenance.html" : "http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/trainlocation/mainte/rosen_maintenance_" + lang + ".html";

	$.when(
		$.getJSON("http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/trainlocation/json/location/now/location_" + _param_rosen + "_now.json?" + now),
		$.getJSON("./master/rosen_name_master.json?" + mstNow),
		$.getJSON("./mainte/rosen_maintenance.json?" + mstNow),
		$.getJSON("http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/webunkou/json/master/ressha_type_master.json?" + mstNow),
		$.getJSON("http://34.46.208.100:8080/proxy?url=https://www3.jrhokkaido.co.jp/webunkou/json/master/eki_master.json?" + mstNow),
		$.get(rosen_html),
